...
export default function Departments({ darkMode }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      const { data } = await axios.get('/departments', { headers: { Authorization: `Bearer ${token}` } })
      setItems(data)
    } catch (err) {
      showToastMessage('Failed to load departments', 'danger')
    } finally {
      setLoading(false)
    }
  }
  ...
  const submit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token')
      await axios.post('/departments', form, { headers: { Authorization: `Bearer ${token}` } })
      setForm({ name: '', code: '', description: '' })
      showToastMessage('Department created successfully', 'success')
      load()
    } catch (err) {
      showToastMessage(err.response?.data?.error || 'Failed to create department', 'danger')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  const updateDepartment = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token')
      await axios.patch(`/departments/${editForm.id}`, editForm, { headers: { Authorization: `Bearer ${token}` } })
      setEditForm(null)
      showToastMessage('Department updated successfully', 'success')
      load()
    } catch (err) {
      showToastMessage(err.response?.data?.error || 'Failed to update department', 'danger')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      const token = localStorage.getItem('token')
      await axios.delete(`/departments/${deleteTarget.id}`, { headers: { Authorization: `Bearer ${token}` } })
      setShowDeleteDialog(false)
      setDeleteTarget(null)
      showToastMessage('Department deleted successfully', 'success')
      load()
    } catch (err) {
      showToastMessage(err.response?.data?.error || 'Failed to delete department', 'danger')
    }
  }
    ...
    <div>
      ...
      <div className={`card p-4 mb-4 shadow-sm border-0 ${darkMode ? 'bg-secondary text-white' : ''}`}>
        <h5 className="card-title mb-3 d-flex align-items-center gap-2">
          <Plus size={20} />
          {editForm ? 'Edit Department' : 'Add New Department'}
        </h5>
        <form onSubmit={editForm ? updateDepartment : submit} className="row g-3">
                  ...
                  {paginated.length > 0 ? paginated.map(item => (
                    ...
                    <tr>
                      <td colSpan="4" className="text-center text-muted py-4">
                        ...
                        <p className="mb-0">No departments found matching your search.</p>
                      </td>
                    </tr>
                  )}
                </tbody>
